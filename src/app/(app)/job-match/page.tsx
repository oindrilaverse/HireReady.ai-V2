...
export default function JobMatchPage() {
  ...
  useEffect(() => {
    async function fetchSuggestions() {
      ...
      try {
        const res = await fetch(getApiUrl(`/jobs/suggested/${latestResume.id}`));
        if (res.ok) {
  ...
  const handleMatch = async () => {
    ...
    try {
      ...
      const matchRes = await fetch(getApiUrl("/jobs/match"), {
        method: 'POST',
      ...
      if (!matchRes.ok) {
        ...
        throw new Error(errData.error || "Failed to match job");
      }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700">
      <header className="text-center space-y-4 pt-8 mb-12">
        <div className="w-16 h-16 rounded-2xl bg-indigo-500/20 flex items-center justify-center mx-auto mb-6 shadow-[0_0_30px_rgba(99,102,241,0.2)] border border-indigo-500/30">
        ...
        {suggestedLoading ? (
          ...
          suggestedJobs.map((job, i) => (
            <motion.div 
              key={i}
              whileHover={{ y: -5 }}
              className="bg-slate-900 border border-slate-800 p-6 rounded-2xl relative overflow-hidden group flex flex-col h-full"
            >
              <div className="absolute top-0 right-0 p-3">
                <span className={`text-[10px] font-bold text-white px-2.5 py-1 rounded-full shadow-lg ${job.matchScore >= 80 ? 'bg-green-500' : job.matchScore >= 60 ? 'bg-yellow-500' : 'bg-red-500'}`}>MATCH {job.matchScore}%</span>
              </div>
              ...
              <div className="mb-6 flex-1">
                <p className="text-[10px] text-slate-500 font-bold mb-2 uppercase tracking-wider">Missing Skills</p>
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {(!job.missingSkills || job.missingSkills.length === 0 || job.missingSkills[0] === "None") && (
                    <span className="text-[10px] bg-green-500/10 border border-green-500/20 text-green-400 px-2 py-0.5 rounded-full">None</span>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 mt-auto">
                <a 
                  ...
                  className="text-xs font-bold bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg flex items-center justify-center flex-1 transition-colors shadow-sm"
                >
                  Apply Now
                </a>
                <button 
                  ...
                  className="text-xs font-bold text-slate-400 hover:text-indigo-400 flex items-center justify-center p-2 border border-slate-700 hover:border-indigo-500/50 hover:bg-indigo-500/10 rounded-lg transition-all"
      ...
      {!result && !loading && (
        ...
        >
          ...
          <label className="block text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2">
            <Search className="w-4 h-4 text-indigo-400" /> Job Description
          </label>
          <textarea 
            rows={12}
        ...
        >
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col items-center justify-center text-center relative overflow-hidden">
              ...
              <p className="text-slate-300 text-sm relative z-10">{result.feedback.summary}</p>
            </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {result.feedback.matchingSkills.map((skill: string, i: number) => (
                    <span key={i} className="px-3 py-1.5 rounded-full text-xs font-medium bg-green-500/10 text-green-400 border border-green-500/20">
                      {skill}
                    </span>
                  ))}
                </div>
